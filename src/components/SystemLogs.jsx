...
const SystemLogs = ({show, setShow, data }) => {
	...
	const columns = [
		{
			...
			return (
				<div className={cell?.IsActionRequired ? "text-danger" : ""}> 
	  ...
	  const Export = ({ onExport }) => <Button size='sm' variant='dark' onClick={e => onExport(e.target.value)} disabled={exporting}>Export</Button>;
  ...
  return (
	<>
      ...
      >
		...
		data={data}
		pagination
		noDataComponent="There are no Logs!"
		//actions={actionsMemo}
		/>
		</Modal.Body>
      </Modal>
